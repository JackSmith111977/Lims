...
- [ ] T-503 实现样品、任务、审核和库存统计（Spec：FR-DASH-001～005；设计：DES-DASHBOARD-001）
  ...
  - [ ] T-503D [S] 完成单元/E2E/远程集成、对抗性审查和一致性门禁（Depends：T-503B～C；Files：`scripts/integration/experiment-processing.mjs`、`scripts/integration/cleanup-experiment-processing.sql`；Verify：临时数据自动清理、全量质量门禁；Review：`REV-DASHBOARD-001`）
