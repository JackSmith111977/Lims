...
| `ENV-505-001` | Environment gate | 演示环境 | SchoolWork 已确认是本系统正式远程数据库，但没有独立的隔离演示边界和授权持久演示账号，不能安全生成 `DEMO_` 远程数据或声称全链路演练通过；本地 `supabase status` 因 Docker Engine 管道不存在而不可用 | 已增加 `T-505C1` 的 `demo:preflight` 安全门禁，当前配置会明确阻断写入；仍需批准 SchoolWork 作为演示环境并完成备份/清理边界，或准备独立隔离项目/分支和三类演示账号，随后执行 `demo-runbook.md`，截图/编号留档并只读核对残留为 0。Free 计划不包含 Supabase Branching，不能把其他暂停项目当作隔离环境 | Open |
