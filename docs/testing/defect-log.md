...
| `ENV-505-001` | Environment gate | 演示环境 | 没有隔离项目和授权持久演示账号，不能安全生成 `DEMO_` 远程数据或声称全链路演练通过；本地 `supabase status` 还因 Docker Engine 管道不存在而不可用 | 需准备隔离 Supabase 项目/分支和三类演示账号，或启动本地 Docker Supabase；随后执行 `demo-runbook.md`，截图/编号留档并只读核对残留为 0 | Open |
