...
- `npm.cmd run demo:preflight`：通过，批准隔离项目为 `test`（project ref：`vrggsiwqttxciaaemhri`），正式项目拒绝写入。
...
- `npm.cmd run demo:preflight` 当前实跑结果：通过，并校验批准状态、隔离标记、项目 ref、应用 URL、生产拒绝列表和占位账号边界。
...
1. 页面级全链路仍未完成：本地应用的 server-only `SUPABASE_SERVICE_ROLE_KEY` 仍指向正式 `SchoolWork`，不能与隔离项目 URL 混用；需由开发者在本地安全配置隔离项目对应密钥后再做登录、角色切换、负向权限、报告追溯和审计页面验证。
