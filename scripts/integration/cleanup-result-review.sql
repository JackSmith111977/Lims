...
begin
  ...
  from auth.users
  where email ~ '^review_[0-9]+_(reviewer|reader)@example[.]invalid$';

  select coalesce(array_agg(id), array[]::bigint[]) into task_ids
  from public.experiment_task where task_code like 'review_%';
  select coalesce(array_agg(id), array[]::bigint[]) into sample_ids
  from public.sample where sample_code like 'review_%';
  select coalesce(array_agg(id), array[]::bigint[]) into project_ids
  from public.research_project where project_code like 'review_%';
  select coalesce(array_agg(id), array[]::bigint[]) into instrument_ids
  from public.instrument where instrument_code like 'review_%';
  select coalesce(array_agg(id), array[]::bigint[]) into method_ids
  from public.experiment_method where method_code like 'review_%';
  select coalesce(array_agg(id), array[]::bigint[]) into review_ids
  from public.result_review where task_id = any(task_ids);
  select coalesce(array_agg(id), array[]::bigint[]) into role_ids
  from public.sys_role where code like 'REVIEW_READER_%';

  delete from public.audit_log
  where operator_id = any(user_ids)
     or (object_type = 'result_review'
         and object_id = any(array(select review_id::varchar from unnest(review_ids) as review_id)));

  -- Review history is immutable in production; this is owner-only test cleanup.
  alter table public.result_review disable trigger user;
  delete from public.result_review where id = any(review_ids);
