...
- [ ] T-505 准备演示数据和演示流程（Spec：所有 P0；设计：DES-DEMO-SCENARIO-001）
  ...
  - [ ] T-505C [S] 在隔离环境生成演示数据并完成全链路演练和清理（Depends：T-503D、T-505A～C；Files：`scripts/integration/demo-seed.sql`、`docs/demo/demo-initialization-evidence.md`；Verify：种子只读统计、P0 流程、截图/编号证据、演练后 `DEMO_` 残留为 0；Review：`REV-DEMO-001`）
