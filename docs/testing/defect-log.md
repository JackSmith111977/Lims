...
| `ENV-505-001` | Environment gate | 演示环境 | 隔离项目 `test` 已建立并完成 23 个迁移、`DEMO_` 合成种子和只读统计，但页面级全链路演练尚未通过；本地 `.env.local` 当前仍指向正式项目，新的运行时门禁已阻止 URL/密钥混用，且临时诊断账号尚未完成 Dashboard 清理 | 已保留 `T-505C1` 的 `demo:preflight` 安全门禁，正式项目仍禁止演示写入；待用户在本地配置隔离项目三件套后重跑 `demo-runbook.md`，完成截图/编号证据、删除临时账号，并只读核对 `DEMO_` 残留为 0。不得把当前数据库种子结果直接等同于页面验收 | Open |
