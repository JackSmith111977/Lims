...
import { loadInstrumentMaintenance } from "@/lib/server/instrument-maintenance";
...
export default async function InstrumentsPage() {
  ...
  const [instruments, canManage] = await Promise.all([
    loadInstruments(context.supabase),
    hasPermission(context.supabase, "resource.manage"),
  ]);
  const initialMaintenance = instruments[0] ? await loadInstrumentMaintenance(context.supabase, String(instruments[0].id)) : [];

  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-8"><div><Link href="/dashboard" className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600">LIMS 工作台</Link><h1 className="mt-2 text-2xl font-semibold">仪器设备档案</h1></div><Link href="/dashboard" className="text-sm text-slate-500 hover:text-slate-700">返回工作台</Link></div></header><section className="mx-auto max-w-7xl space-y-6 px-6 py-8 lg:px-8"><div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 text-sm text-blue-900">设备编号创建后不可修改；报废设备保留历史且不能恢复。维护、维修和校准记录采用追加方式保存。</div><InstrumentsPanel initialInstruments={instruments} initialMaintenance={initialMaintenance} canManage={canManage} /></section></main>;
}
