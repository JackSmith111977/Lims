...
| --- | --- | --- | --- | --- | --- |
| `ENV-503-001` | Environment gate | Supabase CLI 认证 | 当前机器没有 `SUPABASE_ACCESS_TOKEN` 或 `SUPABASE_DB_URL`，`supabase db query --linked` 无法完成 T-503 自动清理；业务断言和浏览器 SQL 清理已有证据 | 用户只在本机配置 CLI token 或完整 `SUPABASE_DB_URL` 后复跑 `test:dashboard-integration`，保留 `dashboardIntegration` 和 `cleanupVerified` 输出 | Open |
