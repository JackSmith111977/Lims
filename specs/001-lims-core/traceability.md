...
| GB/T 40343—2021：文件数据交换 | `FR-DATA-006` | 任务级 CSV/Excel 导入、统一字段校验、`FILE` 来源和导入审计；[`design-data-exchange.md`](design-data-exchange.md) | T-601、`test:data-import`、`experiment-data-access.spec.ts` |
| GB/T 40343—2021：模拟仪器数据 | `FR-DATA-009` | 指定 ACTIVE 设备向任务写入不可变 `INSTRUMENT` 来源数据；`taskId` 路由字段与共享数据载荷字段边界；[`design-simulated-instrument.md`](design-simulated-instrument.md) | T-602、T-602B1、`test:simulated-instrument`、`instrument-access.spec.ts` |
...
| GB/T 40343—2021：设备管理 | `FR-EQUIP-001～007` | 仪器设备档案、维护/维修/校准、状态、负责人、提醒、数据关联和真实仪器接口评估；[`design-instrument-registry.md`](design-instrument-registry.md)、[`design-instrument-maintenance.md`](design-instrument-maintenance.md)、[`RES-REAL-INSTRUMENT-001`](../../docs/kb/RES-REAL-INSTRUMENT-001.md) | T-401、T-402、T-406、T-605、`AC-RESOURCE-001`、`REV-INSTRUMENT-REGISTRY-001`、`REV-INSTRUMENT-MAINTENANCE-001`、`REV-REAL-INSTRUMENT-EVALUATION-001`、`REV-RESOURCE-MANAGEMENT-001`、`test:instrument-integration`、`test:resource-integration`、`instrument-maintenance-validation.test.ts`、`instrument-access.spec.ts` |
...
| GB/T 40343—2021：设施和环境管理 | `FR-ENV-001～006` | 复用实验室/区域主档，维护环境阈值、采集记录和超阈值提醒；真实传感器接口评估并预留；[`design-environment-monitoring.md`](design-environment-monitoring.md)、[`RES-REAL-INSTRUMENT-001`](../../docs/kb/RES-REAL-INSTRUMENT-001.md) | T-105B、T-405、T-605、`AC-RESOURCE-001`、`REV-ENVIRONMENT-MONITORING-001`、`REV-REAL-INSTRUMENT-EVALUATION-001`、`test:environment-integration`、`environment-validation.test.ts`、`environment-access.spec.ts` |
...
| GB/T 40343—2021：报告模板设置 | `FR-SETTING-004`、`FR-REPORT-001～006` | `REPORT_TEMPLATE_` JSON 模板资源、路径编码 PATCH 边界、设置权限/审计和报告生成时不可变模板快照；[`design-report-template.md`](design-report-template.md) | T-603、T-603B1、`REV-REPORT-TEMPLATE-001`、模板单元/E2E/报告集成测试 |
| GB/T 40343—2021：报告电子签名 | `FR-REPORT-007` | 已发布报告的 SHA-256 快照签名、签署人/时间、追加记录和签署审计；受限搜索路径下显式限定 `extensions.digest`；[`design-report-signature.md`](design-report-signature.md) | T-604、T-604B1、`REV-REPORT-SIGNATURE-001`、签名单元/E2E/报告集成测试 |
...
| 毕业设计 P0 演示验收 | 所有 P0 FR、`AC-AUTH-001`、`AC-SAMPLE-001～002`、`AC-TASK-001`、`AC-DATA-001`、`AC-REVIEW-001`、`AC-REPORT-001`、`AC-RESOURCE-001`、`AC-AUDIT-001`、`AC-DASH-001` | 合成数据目录、角色切换、读取 Next.js 实际环境文件的前置检查和逐页演示证据；[`design-demo-scenario.md`](design-demo-scenario.md)、[`demo-runbook.md`](../../docs/demo/demo-runbook.md) | T-505、T-505C1、`REV-DEMO-001`、`check:demo-assets`、`test:demo-preflight`、隔离环境演练记录 |
| 系统测试与缺陷闭环 | 所有 `AC-*`、`BR-*`、`NFR-*` | 测试层级、验收矩阵、负向场景、环境门禁和缺陷复测；[`design-system-test.md`](design-system-test.md)、[`system-test-plan.md`](../../docs/testing/system-test-plan.md) | T-506、`REV-SYSTEM-TEST-001`、`check:system-test-plan`、全量测试报告 |
...
| 工程环境与开发准入 | `NFR-ENV-001`、`NFR-ENV-002`、`NFR-DEV-001`、`AC-ENV-001`、`AC-DEV-001` | SchoolWork 正式远程 Supabase 数据源、`test` 隔离演示数据库、受控浏览器操作、自动化例外、规范读取和变更准入 | T-102F、T-505C1～C3、T-505C、[`remote-supabase-workflow.md`](../../docs/sdd/remote-supabase-workflow.md)、`check-consistency.ps1`、`check:system-test-plan` |
...
| MVP 入口与初始化收尾 | `FR-AUTH-001～006`、`FR-DASH-001～005`、`NFR-SEC-001～002`、`NFR-ENV-001～002`、`NFR-DEV-001`、`AC-DEV-001` | 根入口、登录后工作台、受控首个管理员初始化、无公开注册、旧设计清理；[`design-mvp-closeout.md`](design-mvp-closeout.md)、[`initial-admin-bootstrap.md`](../../docs/ops/initial-admin-bootstrap.md) | T-606～T-609、`bootstrap:admin`、未认证/角色导航测试、`check-consistency.ps1`、`check:system-test-plan` |
| 工作台导航可用性优化 | `FR-AUTH-002`、`FR-DASH-001～005`、`NFR-USE-001`、`AC-DASH-001` | 权限过滤的业务分组导航、当前路径标识、响应式点击区域和键盘可用性；二次优化继续收敛视觉层级和交互密度；[`design-dashboard-navigation.md`](design-dashboard-navigation.md) | T-610～T-616、`dashboard-navigation` 单元/E2E、`dashboard-access.spec.ts`、生产构建和一致性门禁 |
