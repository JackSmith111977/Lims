...
foreach ($path in @($catalogPath, $runbookPath, $specPath)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { [void]$errors.Add("Missing demo asset: $path") }
}

if ((Test-Path -LiteralPath $catalogPath) -and (Test-Path -LiteralPath $runbookPath) -and (Test-Path -LiteralPath $specPath)) {
